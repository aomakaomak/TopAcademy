...
while True:
    action = input(
        "Введите желаемое действие: add - добавить задачу, remove - удалить задачу, list - просмотреть все задачи, exit для выхода: "
    )
    if action == "add":
        while True:
            task = input(
                "Введите задачу. Для возвращения к выбору действия нажмите back: "
            )
            if task != "back":
        ...
        while True:
            try:
                remove = input(
                    "Введите номер задачи начиная с 0. Для возвращения к выбору действия нажмите back: "
                )
                if remove != "back":
                    remove = int(remove)
                    list_task.pop(remove)
                    print(
                        f"Задача {list_task[remove]} под номером {remove} успешно удалена."
                    )
                    print(list_task)
                else:
                    break
